...
def read_cifar100(filename_queue, coarse_or_fine='fine'):
    ...
    if coarse_or_fine == 'fine':
        result.label = fine_label # 100 classes
    else:
        result.label = coarse_label # 20 classes
...
def distorted_inputs(data_dir, cifar10or20or100,  batch_size):
    ...
    # resize images
    sesized_image = tf.image.resize_images(images=reshaped_image, size=[height, width])


    # Image processing for evaluation.
    # Crop the central [height, width] of the image.
    padded_image = tf.image.resize_image_with_crop_or_pad(sesized_image, height + 4, width + 4)
...
def inputs(eval_data, data_dir, cifar10or20or100, batch_size):
    ...
    # resize images
    resized_image = tf.image.resize_images(images=reshaped_image, size=[height, width])

    # Image processing for evaluation.
    # Crop the central [height, width] of the image.
    padded_image = tf.image.resize_image_with_crop_or_pad(resized_image, height, width)

    # Subtract off the mean and divide by the variance of the pixels.
    float_image = tf.image.per_image_standardization(padded_image)
